...
def svg_groups(paths, prefix='ink'):
    ...
    for path in paths:
        ...
        result.append(f'<path id="{escape(prefix)}-{path["id"]:03d}" '
                      f'fill="#000000" fill-rule="nonzero" d="{data}"/>')
    return '\n'.join(result)


def svg_document(paths, width, height, prefix='ink', background=False):
    bg = f'<rect width="{width}" height="{height}" fill="#ffffff"/>' if background else ''
    return (f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" '
            f'viewBox="0 0 {width} {height}"><title>{prefix}: traced black regions</title>'
            f'{bg}{svg_groups(paths, prefix)}</svg>\n')
