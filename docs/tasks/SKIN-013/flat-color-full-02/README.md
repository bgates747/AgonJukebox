...
Accepted by the Author on 2026-09-10 as a "good-enough draft to test with",
with a requested progress commit. This image is the test baseline; the broader
authoring work remains open. Runtime integration and target testing have not
been performed for this draft.
...
Inkscape 1.2.2 (b0a8486541, 2022-12-01) exported 50167
...
```bash
.venv/bin/python docs/tasks/SKIN-013/flat-color-full-02/colorize_shapes.py \
  ...
  --params docs/tasks/SKIN-013/flat-color-full-02/parameters.json \
...
## Verification and correction from candidate 01

One Simplify invocation selects all shape paths; an isolated
`/options/simplifyindividualpaths/value=1` preference applies tolerance against
each path's bounds. This avoids the accelerated strength caused by hundreds
of rapid consecutive Simplify invocations in candidate 01. That failed candidate
is retained as evidence. The relevant upstream implementations are
[simplification acceleration](https://gitlab.com/inkscape/inkscape/-/blob/INKSCAPE_1_2_2/src/path/path-object-set.cpp)
and [individual path bounds](https://gitlab.com/inkscape/inkscape/-/blob/INKSCAPE_1_2_2/src/path/path-simplify.cpp).

[verify_colorized.py](verify_colorized.py) checks this candidate independently:
palette, alpha and dimensions; source hashes; shape bounds before/after
simplification; agreement with Inkscape on uniform interior pixels; relative
curve, transform and hole handling; optional byte-identical rebuild and output
overwrite rejection. Run with the project .venv:

```bash
.venv/bin/python docs/tasks/SKIN-013/flat-color-full-02/verify_colorized.py \
  docs/tasks/SKIN-013/flat-color-full-02 --repeat
```

The verifier retains an existing verification record and prints current checks. The saved
[verification.json](verification.json) passes all checks, including a repeat
build. Maximum simplified bounding-edge movement is 0.203 target pixels;
78,782 uniform interior pixels agree with Inkscape's independent renderer.
