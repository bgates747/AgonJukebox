...
def main():
    ...
    profile.joinpath('preferences.xml').write_text('<inkscape version="1"><group id="options">'
        ...
        '</group></inkscape>')
    # One Simplify invocation avoids Inkscape's time-based acceleration of
    # repeated invocations. The preference above uses each path's own bounds.
    actions = 'select-by-id:' + ','.join(r['id'] for r in records) + ';path-simplify'
    ...
    save_json(out / 'simplification-command.json', {'argv': argv, 'stderr': proc.stderr.strip(),
              ...
              'invocations': 1, 'simplifyindividualpaths': True})
...
```bash
.venv/bin/python {os.path.relpath(out, Path.cwd())}/colorize_shapes.py \\
  ...
  --params {os.path.relpath(out, Path.cwd())}/parameters.json \\
