...
### AGM build and reference playback restored

Installed and pinned `yt-dlp` 2026.7.4, then repaired the historical AGM media
pipeline so it uses the active Python interpreter and project-local resources.
`agm_make.py` now creates its required directories, reads `Agon64.gpl` from the
`agon-utils` submodule, and invokes the submodule's RLE2, SZIP, and TurboVega
executables without relying on global `PATH` entries.

Successfully exercised the complete PC-side pipeline:

1. downloaded the configured Battle of Yavin video and audio;
2. converted the audio to 15,360 Hz unsigned 8-bit mono PCM;
3. extracted and Bayer-converted video frames;
4. applied delta framing followed by RLE2 and SZIP compression;
5. packaged synchronized audio and video into AGM; and
6. played the result through the Pygame reference player.

The first successful baseline contained 1,200 frames at 240x104 and 10 fps. A
subsequent full run also completed with 2,880 frames at 320x136 and 24 fps for
120 seconds. The latter output demonstrates that the recovered encoder and
local codec tools operate end to end.

Updated `agm_play.py` to select the generated SRLE2 baseline and to use the
submodule's local decompression executables. Actual synchronized PC playback
was confirmed by the user.

The 15,360 Hz audio rate was traced through the assembly player. It is not a
file-format constant: the player divides the header sample rate into 60 UART
uploads per second. Rates divisible by 60 avoid integer timing loss, while the
historical UART test identifies 960 bytes per 1/60-second tick as the safe
upper transfer size. At 15,360 Hz, each audio upload is exactly 256 bytes.

One transient YouTube download failure was observed. The script suppresses
`yt-dlp` diagnostics, making such failures opaque. Repeating the exact command
with visible output succeeded; warnings noted that the `mp4` format selector is
old-fashioned and that no JavaScript runtime is configured.
