...
def build(output: Path):
    ...
    cleanup += b''.join(clear_buffer(a['id']) for a in assets)+clear_buffer(0x21f0)+clear_buffer(0x2200)
    ...
    def text_widget(name,x,y,n,bg=BLACK,fg=TEXT,rect=None,slot='text'):
        rect = rect or (x,y,n*5,8)
        data = context(1)+font(0x21f0)
        slots = {'bg':len(data)+2}
        data += fill(*rect,bg)
        slots['fg'] = len(data)+2
        ...
        specs[name] = {'kind':'text','x':x,'y':y,'n':n,'bg':bg,'fg':fg,'rect':rect}
    ...
    for row in range(10):
        text_widget('w_row'+str(row),85,90+row*12,60,rect=(81,88+row*12,350,12))
    ...
    generated += ['art_playing: equ art_pause','art_paused: equ art_play',
                  f'ui_normal_fg: equ {colour(TEXT)}',f'ui_selected_bg: equ {colour(GOLD)}',
    ...
    def preview(fields):
        ...
        for name,spec in specs.items():
            ...
            for index,char in enumerate(value):
                mask = glyphs[ord(char)].convert('L').crop((0,0,5,8))
                result.paste(fg,(x+index*5,y,x+index*5+5,y+8),mask)
        return result
    ...
    for spec in specs.values():
        if spec['kind']=='text' and spec['n'] != 8:
            ...
            for y in range(spec['y'],spec['y']+8):
                for x in range(spec['x'],spec['x']+min(16,spec['n'])*5,3):
                    points.add((x,y))
    ...
    info = {'format':'artdeco-authoring-test1','source_sha256':sha(source.read_bytes()),
        ...
        'screen':[512,384],'font_cell':[5,8],'browser_columns':60,'browser_rows':10,
        ...
        'agnb_bytes':len(packed),'font_bytes':2048,'static_command_bytes':len(static),
