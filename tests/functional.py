...
def prepare(destination: Path, skin: str = "base") -> None:
    ...
    subprocess.run(
        ["ez80asm", "../../tests/asm/" + ({"artdeco":"artdeco_check.asm","seventies":"seventies_check.asm"}.get(skin,"livecheck.asm")),
...
def check(log: Path, skin: str = "base") -> None:
    ...
    if skin in ("artdeco","seventies") and "LIVE_FONT_PIXELS_PASS" not in output:
...
def main() -> None:
    ...
    prepare_cmd.add_argument("--skin", choices=["base", "artdeco", "seventies"], default="base")
    check_cmd = commands.add_parser("check", help="validate a completed emulator log")
    check_cmd.add_argument("log", type=Path)
    check_cmd.add_argument("--skin", choices=["base", "artdeco", "seventies"], default="base")
