...
def compute_frame_difference(oldFinal: bytes, newFinal: bytes) -> bytes:
    """
    8-bit difference: 0 => unchanged, else new pixel index
    """
...
def dither_diff_test():
    ...
    from start index to end index inclusive. For each .png:
      ...
      - Difference vs. old final => difference array => difference .png in diff_dir
    """
    ...
    # ------------------------------------------------------------------
    # 0) Clear diff_dir of old files so we don't mix old & new runs
    # ------------------------------------------------------------------
    for filename in os.listdir(diff_dir):
        path = os.path.join(diff_dir, filename)
        if os.path.isfile(path):
            os.remove(path)
        # If you want to remove subdirectories too, uncomment:
    ...
    # 3) Main loop
    palette_file = "/home/smith/Agon/mystuff/assets/images/palettes/Agon63.gpl"  # or your path
    frame_count = 0
