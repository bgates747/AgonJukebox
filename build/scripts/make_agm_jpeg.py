...
AGM_UNIT_CMP_JPG = 0b00010000  # Bit  4: JPEG compression (same bits as jpeg)

# Final video unit mask: video type OR JPEG compression.
VIDEO_MASK = AGM_UNIT_TYPE | AGM_UNIT_CMP_JPG
# --------------------------------------------------------------------

def compress_frame_data(frame_bytes, frame_idx, total_frames, width, height, quality, optimize):
    """
    Compress the raw frame data using JPEG compression.
    
    Parameters:
      frame_bytes (bytes): The raw frame data. For a grayscale image, 
                           this should be width*height bytes (8-bit per pixel).
      frame_idx (int): Index of the current frame (for status printing).
      total_frames (int): Total number of frames (for status printing).
      width (int): Image width (default 320).
      height (int): Image height (default 136).
      quality (int): JPEG quality setting (1-95; lower means higher compression).
      
    Returns:
      bytes: The JPEG-compressed frame data.
    """
    # Create a grayscale PIL image from the raw data.
    # 'L' mode means 8-bit pixels, black and white.
    try:
        image = Image.frombytes("L", (width, height), frame_bytes)
    except Exception as e:
        raise ValueError(f"Error creating image from raw data: {e}")
    ...
    # Print progress (overwriting the current line).
    print(
        f"\rJPEG compressed frame {frame_idx + 1} of {total_frames}, "
...
def make_agm_jpeg(frames_directory,target_audio_path,target_agm_path,target_width,target_height,frame_rate,target_sample_rate,chunksize,quality,optimize):
    """
    ...
    with open(target_agm_path, "wb") as agm_file:
        ...
        for sec in range(total_secs):
            ...
            for _ in range(frame_rate):
                ...
                compressed_frame_bytes = compress_frame_data(frame_bytes, frame_idx - 1, total_frames, target_width, target_height, quality, optimize)
