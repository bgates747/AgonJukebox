...
def compress_audio_data(audio_bytes):
    """
    Compress the raw audio data using szip.
    Parameters:
      audio_bytes (bytes): The raw audio data (1 second worth).
    Returns:
      bytes: The compressed audio data.
    """
    ...
    try:
        ...
        temp_szip = tempfile.NamedTemporaryFile(delete=False)
        temp_szip.close()
        temp_szip_path = temp_szip.name

        # Run szip to compress audio data.
        subprocess.run(["szip", "-b41o3", raw_path, temp_szip_path], check=True)

        with open(temp_szip_path, "rb") as f:
            compressed_bytes = f.read()
    finally:
        # Clean up temporary files.
        if os.path.exists(temp_raw.name):
            os.remove(temp_raw.name)
        if os.path.exists(temp_szip_path):
            os.remove(temp_szip_path)
...
def make_agm_srle2(
    ...
    Additionally, writes a CSV file recording aggregated per-second data.
    The CSV file will have a header with frame_size, frame_rate, and audio_rate,
    and then for each second, a row with time (sec), compressed_video_bytes, and compressed_audio_bytes.
    ...
    total movie duration in seconds (total_frames/frame_rate), the file is segmented per frame,
    ...
    def compress_audio_data(audio_bytes):
        ...
        try:
            ...
            # Run szip to compress audio data.
            subprocess.run(["szip", "-b41o0", raw_path, temp_szip_path], check=True)
    ...
    print(f"Video ~{video_secs_float:.2f}s, Audio ~{audio_secs_float:.2f}s => Merging {total_frames} frames total.")
    ...
    # Prepare aggregation arrays for video and audio (per second).
    aggregated_video_bytes = [0] * total_secs
    aggregated_audio_bytes = [0] * total_secs
    ...
    with open(csv_filename, "w") as csv_file:
        ...
        csv_file.write("time_sec,compressed_video_bytes,compressed_audio_bytes\n")
        ...
        with open(target_agm_path, "wb") as agm_file:
            ...
            segment_size_last = 0
            # Number of audio samples corresponding to one frame.
            ...
            for frame_index in range(total_frames):
                ...
                # ---------------- AUDIO UNIT (with compression) ----------------
                seg_buffer.write(struct.pack("<B", AUDIO_MASK))
                # Calculate the audio samples for this frame.
                ...
                # Compress the audio for this frame.
                compressed_audio = compress_audio_data(unit_audio)
                offset = 0
                while offset < len(compressed_audio):
                    chunk = compressed_audio[offset: offset + chunksize]
                    offset += len(chunk)
                    seg_buffer.write(struct.pack("<I", len(chunk)))
                    seg_buffer.write(chunk)
                # End of audio unit.
                ...
                # Aggregate sizes into the corresponding second.
                ...
                if second_index < total_secs:
                    ...
                    aggregated_audio_bytes[second_index] += len(compressed_audio)

        # Write CSV rows aggregated by second.
        for sec in range(total_secs):
            csv_file.write(f"{sec},{aggregated_video_bytes[sec]},{aggregated_audio_bytes[sec]}\n")
